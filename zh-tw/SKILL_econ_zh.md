...
執行任何動作前，先檢查 `schedule.json` 是否存在且包含有效的 queue：

```
如果 schedule.json 不存在 或 queue 為空：
    → 執行 Bootstrap（以下 Step 2–4）
否則：
    → 顯示現有排程，繼續正常運作
```

這表示用戶不需要記任何額外的初始化指令——skill 在第一次使用時自動初始化，schedule.json 遺失或損毀時也會自動修復。
...
用戶提供兩項資訊後，**立刻執行 Step 2–4，不需要等待用戶任何進一步操作**。到 Step 4 結束時 skill 已完全啟動，不需要額外的啟動指令。
